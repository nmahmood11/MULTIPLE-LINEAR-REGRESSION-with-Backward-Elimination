...
dataset = pd.read_csv(r"C:\Users\Noman\Desktop\DS Parkash\Datasetfile\MLR\50_Startups.csv")
...
# comparing the dataset with dummy variable & we are done with dummy variable 
...
#create the vector of prediction is called as y_pred & these all are 10 predicted profits & also  let me open y_test which is actual observation or real profit
#compare the profit of real profit is y_test & predicted profit is y_pred which build by model 
#we almost got accurate prediction & we can say that our model did quite good job & we build the multilinear regression model
#Evaluate that which variable is highest impact or very good corelation to the dependent variable
